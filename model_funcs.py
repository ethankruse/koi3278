...
def loadisos():
    ...
    -------
    isobundle : tuple
        (magobs, magerr, maglam, magname, interps, limits, fehs,
         ages, maxmasses, wdmagfunc)
    """
    # get the inputs
    from inputs import (isodir, inds, magname, magobs, magerr, maglam,
                        wdfiles, wdinds)
    import warnings
    ...
    isobundle = (magobs, magerr, maglam, magname, interps, limits, fehs,
                 ages, maxmasses, wdmagfunc)
    return isobundle
...
def initrange(p):
    ...
    if len(p) == 14:
        return np.array([2.41373687e-04, 2.15625144e-03, 5.42134862e-05,
                         4.95601162e-02, 4.33065931e-02, 1.19787163e-01,
                         1.03564141e-01, 9.25556734e-02, 2.34386318e-01,
                         2.34386318e-01, 2.34386318e+01, 1.34386318e-03,
                         5.26059466e+00, 1.00059466e-02])
    if len(p) == 16:
        return np.array([2.41373687e-04, 2.15625144e-03, 5.42134862e-05,
                         4.95601162e-02, 4.33065931e-02, 1.19787163e-01,
                         1.03564141e-01, 9.25556734e-02, 2.34386318e-01,
                         2.34386318e-01, 2.34386318e+01, 1.34386318e-03,
                         5.26059466e+00, 1.00059466e-02, 0.1, 0.1])
...
def light_curve_model(t, p, isobundle, npert=1):
    ...
    p : ndarray
        ...
        distance, systematic magnitude errors, dust scale height,
        and the total extinction A_lambda.
...
def logprior(p, isobundle):
    ...
    if (period < 0. or e < 0. or e >= 1. or a < 0. or u10 + u11 >= 1 or
            u20 + u21 >= 1 or M2 < 0. or M2init < 0. or height < 0. or
            M1 < 0. or dist < 0. or syserr < 0.):
        return -np.inf
...
def loglikeli(p, t, f, ferr, cuts, crowding, isobundle,  minimize=False,
    ...
    # convert from absolute to apparent magnitude given the extinction
    # parameters and the galatic latitude of KOI-3278
    magmodel = (5. * np.log10(dist / 10.) + alammult * maglam *
                (1. - np.exp(-dist * np.sin(10.2869 * np.pi / 180.) /
                             height)) + mags)

    # regular chi-square plus a penalty for large systematic error
    totchisq += (np.sum((magmodel - magobs)**2. / (magerr**2. + syserr**2.)) +
                 np.sum(np.log(magerr**2. + syserr**2.)))
