...
for ii in np.arange(len(infiles)):
    ...
    tcads = np.loadtxt(infiles[ii],skiprows=1,unpack=True,usecols=(1,))
    for jj in tcads:
...
# get the light curve
time,flux,fluxerr,cad,quart,qual = preparelc(KIC,usepdc=usepdc,fill=True)
...
# will have to handle errors (e.g. transits near the beginning or end of observation) before generalizing this for use with other systems
if lorig != len(cads):
