...
from model_funcs_public import msage, kepler_problem, isointerp, loadisos, getwdmodels
...
for ii in np.arange(nparams+1):
    ...
    for jj in uwalkers:
        ...
        if ii < nparams:
            # if this chain is really long, cut down on plotting time by only plotting every tenth element
...
# used to feed back to the other file for initrange, and plotting the best fit model for publication figures
# TODO: file name
best = x[0,:]
...
if texout is not None:
    ...
    if inferredparams:
        ...
        # set up the output
        results = np.zeros((len(FeH),len(isointerp(M[0],FeH[0],age[0],isobundle))))
